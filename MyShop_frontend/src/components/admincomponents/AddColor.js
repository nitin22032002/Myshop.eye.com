    ...
    const handelClick=async()=>{
       var err=false;
        ...
        if(isEmpty(getPicture.filename)){
            ErrorMessage(`Color Image Not Be Empty`)
            err=true;
        }
        if(!err){
    ...
    return(
        <div className={classes.root}>
            <div className={classes.subdiv}>
                <Grid container>
                    <Grid item xs={12}>
                        <h3 className={classes.heading}><img alt="..."  src="/lenslogo.png" style={{width:50,height:20}} /> Add {"Color"}</h3>
                    </Grid>
                    <Grid item xs={12}>
                        <TextField variant="outlined" className={classes.items} label={`Color Name`} fullWidth onChange={(event)=>setShapeName(event.target.value)} />
                    </Grid>
                    <Grid item xs={12}>
      ...
      <label htmlFor="contained-button-file">
        ...
        <Avatar alt="Your Image Not Uploaded"  className={classes.items} style={{height:50,width:50}}  variant="rounded" src={getPicture.filename} />
                ...
                <Grid item xs={12}>
                    <Button variant="contained"  className={classes.items} style={{background:"#636e72",color:'#55E6C1'}} fullWidth onClick={()=>handelClick()}>Add {"Color"}</Button>
                </Grid>
                </Grid>
                </div>
        </div>

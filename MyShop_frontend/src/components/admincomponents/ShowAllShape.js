...
import AddShape from './AddShapes'
...
export default function DisplayAllShape(props){
    ...
    const handelClick=async()=>{
         if(isEmpty(getShapeName)){
             ErrorMessage(`Shape Name Not be Empty`)
         }
         else{
             var body={id:getShapeId,name:getShapeName,tablename:"shapes",status:getStatus}
    ...
    const handledelete=async(x)=>{
        ...
        Swal.fire(
            ...
            imageHeight:50,
            title: 'IndoForest\nAre you sure?',
            text: `You will not be able to recover this Shape record!`,
            icon: 'warning',
            ...
            if (result.isConfirmed) {
              result=await postData("attribute/deleteShape",{id:id,img:img,tablename:"shapes"})
              if(result){
                swal(
                  <div><img alt="..."   src="/lenslogo.png" style={{width:100,height:50}}/><h3>IndoForest</h3></div>
                  ,{
    ...
    const handleeClickOpen=(x)=>{
        seteOpen(true);
        setShapeId(x[`shapeid`]);
        setShapeName(x[`shapename`])
        setStatus(x.status)
    ...
    const handleImageSave=async()=>{
        ...
        formdata.append("tablename","shapes")
    ...
    const EditDialogBox=()=>{    
          ...
          <div>
            ...
            <div className={classes.subdiv}>
                <Grid container>     <Grid item xs={12}>
                        <h3 className={classes.heading}><img alt="..."   src="/lenslogo.png" style={{width:50,height:20}} /> Edit {"Shape"}</h3>
                    </Grid>               <Grid item xs={12}>
                        <TextField value={getShapeName} variant="outlined" className={classes.items} label={`Shape Name`} fullWidth onChange={(event)=>setShapeName(event.target.value)} />
                    </Grid>
                    <Grid item xs={12}>
      ...
      <label htmlFor="contained-button-file">
        ...
        <Avatar alt="Your Image Not Uploaded"  className={classes.items} style={{height:50,width:50}}  variant="rounded" src={getPicture.filename} />
                ...
                <Grid item xs={12}>
                    <Button variant="contained"  className={classes.items} style={{background:"#636e72",color:'#55E6C1'}} fullWidth onClick={()=>handelClick()}>Edit {"Shape"}</Button>
                </Grid>
    ...
    size="meadium"
    startIcon={<AddIcon/>}
    onClick={()=>props.setComponent(<AddShape/>)}
  >
    Add Shape
  </Button>
          </div>}
